...
def benchmark_image_size(model: torch.nn.Module, device: torch.device, 
                             model_name: str, step: int, num_runs: int = 3,
                             num_warmup: int = 3):
    ...
    print(f"{'Image Size':<15} {'Pixels':<15} {'Time (ms)':<25} {'Memory (MB)':<25} {'Throughput (MP/s)':<18}")
    print(f"{'-'*98}")
    
    for result in scaling_results:
        time_str = f"{result['avg_time_ms']:.2f} \u00b1 {result['std_time_ms']:.2f}"
        mem_str = f"{result['avg_memory_mb']:.1f} \u00b1 {result['std_memory_mb']:.1f}"
        print(f"{result['image_size']:<15} {result['num_pixels']:<15,d} "
...
def create_scaling_plots(all_scaling_results_64: Dict[str, List[Dict]], 
    ...
    fig_time = make_subplots(
        rows=1, cols=2, 
        subplot_titles=("Step 64", "Step 68"),
    ...
    fig_time.update_yaxes(title_text="Time (ms)", range=[min_time, max_time], row=1, col=1)
    fig_time.update_xaxes(title_text="Image Width (pixels)", row=1, col=2)
    fig_time.update_yaxes(title_text="Time (ms)", range=[min_time, max_time], row=1, col=2)
    ...
    fig_mem.update_yaxes(title_text="Memory (MB)", range=[min_mem, max_mem], row=1, col=1)
    fig_mem.update_xaxes(title_text="Image Width (pixels)", row=1, col=2)
    fig_mem.update_yaxes(title_text="Memory (MB)", range=[min_mem, max_mem], row=1, col=2)
...
def main():
    ...
    for model_name, model in models.items():
        # Test 1: Step 64
        scaling_results_64 = benchmark_image_size(model, device, model_name=model_name, step=64, num_runs=3)
        all_scaling_results_64[model_name] = scaling_results_64
        
        # Test 2: Step 68
        scaling_results_68 = benchmark_image_size(model, device, model_name=model_name, step=68, num_runs=3)
