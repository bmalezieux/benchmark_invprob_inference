...
def create_plot_layout(title, xaxis_title, yaxis_title, show_legend=False, legend_y=-0.2):
...
def plot_psnr_vs_metric(parquet_df, output_dir, metric='time', filename='psnr_vs_time.html'):
    ...
    fig.update_layout(**create_plot_layout(title, xaxis, 'PSNR (dB)', show_legend=show_legend, legend_y=-0.15))
...
def plot_gpu_memory_from_parquet(parquet_df, output_dir):
    ...
    for name, col, color in [('Gradient', 'objective_gradient_memory_peak_mb', 'blue'),
...
def create_dashboard(results_path, result_name):
    ...
    required_files = ['psnr_vs_time.html', 'psnr_vs_iteration.html', 
                     'time_breakdown_stacked.html', 'gpu_memory_gradient_denoiser.html']
...
def visualize_general_results(output_dir, results_dir='results_images'):
    ...
    # Create all plots
    plot_psnr_vs_metric(parquet_df, results_path, metric='time', filename='psnr_vs_time.html')
    plot_psnr_vs_metric(parquet_df, results_path, metric='iteration', filename='psnr_vs_iteration.html')
