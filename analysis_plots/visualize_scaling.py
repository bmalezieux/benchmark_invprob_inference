...
def read_parquet_data(output_dir):
    ...
    def get_total_gpus(row):
        ...
        gpus_per_node = int(gres.split(":")[1]) if isinstance(gres, str) and ":" in gres else 1
    ...
    def create_config_label(row):
        ...
        if nodes == 1 and tasks == 1: return "Single GPU"
    ...
    img_size_col = "p_dataset_image_size" if "p_dataset_image_size" in parquet_df.columns else "p_dataset_img_size"
...
def plot_generic(scaling_dict, output_path, title, y_label, metric_cfg, y_range=None):
    ...
    if isinstance(metric_cfg, str):
        ...
        for idx, (img_size, df) in enumerate(sorted(scaling_dict.items())):
            label = f"Image {img_size}×{img_size}" if isinstance(img_size, (int, float)) else str(img_size)
            fig.add_trace(go.Scatter(
                x=df["ngpu"], y=df[metric_cfg], mode="lines+markers", name=label,
                line=dict(width=4, color=colors[idx % len(colors)]), marker=dict(size=12),
        ...
        for col, (color, label) in metric_cfg.items():
            fig.add_trace(go.Scatter(
                x=df["ngpu"], y=df[col], mode="lines+markers", name=label,
    ...
    if all_ngpus:
        ...
        fig.add_trace(go.Scatter(
            x=all_ngpus, y=ideal, mode="lines", name=name,
            line=dict(width=3, color="black", dash="dash"), hoverinfo="skip"
        ))
...
def visualize_scaling(output_dir, results_dir="results_images"):
    ...
    try:
        ...
        plot_generic(scaling_dict, out_path / "parallel_efficiency.html", 
                    "Parallel Efficiency", "Efficiency (%)", "efficiency", y_range=[0, 110])
        plots_created.append("parallel_efficiency.html")

        if len(scaling_dict) == 1:
            # Single Image Case: Combined Plot
            cfg = {"speedup": ("green", "Total"), "gradient_speedup": ("blue", "Gradient"), "denoiser_speedup": ("orange", "Denoiser")}
            plot_generic(scaling_dict, out_path / "combined_strong_scaling.html", 
                        "Strong Scaling Breakdown", "Speedup", cfg)
            plots_created.insert(0, "combined_strong_scaling.html") # Put first
        else:
            # Multi Image Case: Separate Plots
            for metric, title in [("speedup", "Strong Scaling"), ("gradient_speedup", "Gradient Scaling"), ("denoiser_speedup", "Denoiser Scaling")]:
                fname = f"strong_scaling_{metric.split('_')[0]}_speedup.html" if '_' in metric else "strong_scaling_speedup.html"
