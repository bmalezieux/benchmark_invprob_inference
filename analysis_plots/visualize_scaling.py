...
def read_parquet_data(output_dir):
    ...
    if "p_dataset_image_size" in parquet_df.columns:
        ...
        raise KeyError(
            "Could not find image size column (tried 'p_dataset_image_size' and 'p_dataset_img_size')"
        )
    ...
    config_info = (
        parquet_df.groupby(["config_label", "ngpu"]).size().reset_index(name="count")
    )
    config_info = config_info.sort_values("ngpu")
    for _, row in config_info.iterrows():
        print(
            f"  - {row['config_label']:12s} ({row['ngpu']:2d} GPUs): {row['count']} data points"
        )
...
def calculate_scaling_metrics(parquet_df):
    ...
    for img_size in sorted(filtered_df[img_size_col].unique()):
        ...
        if len(scaling_data) > 0:
            ...
            baseline_time = scaling_df[scaling_df["ngpu"] == scaling_df["ngpu"].min()][
                "total_time"
            ].values[0]
            baseline_gradient_time = scaling_df[
                scaling_df["ngpu"] == scaling_df["ngpu"].min()
            ]["gradient_time"].values[0]
            baseline_denoiser_time = scaling_df[
                scaling_df["ngpu"] == scaling_df["ngpu"].min()
            ]["denoiser_time"].values[0]
            baseline_ngpu = scaling_df["ngpu"].min()

            scaling_df["speedup"] = baseline_time / scaling_df["total_time"]
            scaling_df["gradient_speedup"] = (
                baseline_gradient_time / scaling_df["gradient_time"]
            )
            scaling_df["denoiser_speedup"] = (
                baseline_denoiser_time / scaling_df["denoiser_time"]
            )
            scaling_df["ideal_speedup"] = scaling_df["ngpu"] / baseline_ngpu
            scaling_df["efficiency"] = (
                scaling_df["speedup"] / scaling_df["ideal_speedup"]
            ) * 100
...
def plot_scaling_metric(
    ...
    for idx, (img_size, scaling_df) in enumerate(sorted(scaling_dict.items())):
        ...
        text_labels = [
            f"{val:.2f}x" if "speedup" in metric_col else f"{val:.1f}%"
        ...
        fig.add_trace(
            go.Scatter(
                ...
                hovertemplate="<b>Image size:</b> "
                + str(img_size)
                + "<br>"
                + "<b>Config:</b> "
                + scaling_df["config_label"]
                + "<br>"
                + "<b>GPUs:</b> %{x}<br>"
    ...
    all_ngpus = sorted(
        set(
            [
                ngpu
                for scaling_df in scaling_dict.values()
                for ngpu in scaling_df["ngpu"]
            ]
        )
    )
    ...
    if "speedup" in metric_col or metric_col == "efficiency":
        ...
        fig.add_trace(
            go.Scatter(
                ...
                hovertemplate=(
                    "<b>Ideal:</b> %{y:.2f}<extra></extra>"
                    if metric_col != "efficiency"
                    else "<b>Ideal:</b> 100%<extra></extra>"
                ),
            )
        )
...
def visualize_scaling(output_dir, results_dir="results_images"):
    ...
    for img_size, scaling_df in sorted(scaling_dict.items()):
        ...
        print(
            scaling_df[
                ["config_label", "ngpu", "total_time", "speedup", "efficiency"]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Visualize scaling benchmark results from parquet file"
    )
    parser.add_argument(
        "output_dir",
        nargs="?",
        default="outputs/scaling/highres_color_image",
