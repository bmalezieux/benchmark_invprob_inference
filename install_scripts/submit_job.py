...
def main():
    ...
    if not config_path.exists():
        # Try relative to repository root first, then install_scripts
        script_dir = Path(__file__).resolve().parent
        repo_dir = script_dir.parent
        candidate = repo_dir / args.config
        if candidate.exists():
            config_path = candidate
        else:
            candidate = script_dir / Path(args.config).name
            if candidate.exists():
                config_path = candidate
    ...
    if args.local:
        ...
        job = executor.submit(run_simulation, config_path)
