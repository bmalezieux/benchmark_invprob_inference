...
def load_shepp_logan_image(img_size: int = 256, grayscale: bool = True, device: str = "cpu", 
    ...
    if original_cache_file.exists():
        ...
        img_tensor = torch.load(original_cache_file, map_location='cpu', weights_only=True)
    else:
...
class Dataset(BaseDataset):
    ...
    def __init__(self, img_size=256, num_operators=2, num_angles=180, 
                 noise_level=0.05, circle=True, seed=42):
    ...
    def _create_operator_factory(self, device: str, angles_list: list, noise_sigma: float):
